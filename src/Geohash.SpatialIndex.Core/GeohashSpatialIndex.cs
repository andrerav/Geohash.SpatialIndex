...
namespace Geohash.SpatialIndex.Core
{
	...
	public class GeohashSpatialIndex<T> : IGeohashSpatialIndex<T>
	{
		...
		///<inheritdoc/>
		public (string, GeohashIndexEntry<T>) Insert(Geometry geom, T value)
		{
			var key = _geohasher.Encode(geom, _precision);
			var entry = new GeohashIndexEntry<T>
			{
				Geom = geom,
				Value = value
			};
		...
		///<inheritdoc/>
		public IEnumerable<GeohashIndexEntry<T>> Query(Geometry geom)
		{
			return Query(_geohasher.Encode(geom, _precision)).Item1;
		}

		///<inheritdoc/>
		public (IEnumerable<GeohashIndexEntry<T>>, string) Query(Geometry geom, int minimumHits = 1, bool lockPrecision = false, T exclude = default)
		{
		...
		///<inheritdoc/>
		public (IEnumerable<GeohashIndexEntry<T>>, string) Query(string hash, int minimumHits = 1, bool lockPrecision = false, T exclude = default)
		{
			IEnumerable<GeohashIndexEntry<T>> entries;
			while (true)
			{
